...
Define MsDialog oDlg Title "Acesso de Usuários" From aSize[7],0 to aSize[6],aSize[5] of oMainWnd PIXEL
         ...
         If Len(aMnuRet)#0
            For j:=1 To Len(aMnuRet)
               ...
               If lAppMod
                  ...
                  WKMODULOS->CODMODULO := aMnuRet[j][2] //Código do módulo
   ...
   While !Ft_FEof() 
      ...
      ElseIf "MENUITEM " $ Upper(cAux)
         ...
         If AllTrim(cRotDe) <= AllTrim(cFuncao) .AND. AllTrim(cRotAte) >= AllTrim(cFuncao)
            ...
            WKACESSO->CODUSMOD  := aUsers[i][1][1]+cMod //Código do user + Código do módulo
            WKACESSO->USER      := aUsers[i][1][4] //Nome do user
            WKACESSO->MODULO    := retModulo(Val(cMod)) //Nome do Módulo
